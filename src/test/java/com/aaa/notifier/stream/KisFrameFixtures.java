 ...
 * ws-03-해외주식실시간체결.md}(2026-06-24 실측)의 "실제 수신 예시" 절에서 그대로 옮긴 것이다. 필드 수(47/26)와 본 SPEC이 소비하는 인덱스 위치는
 ...
 *
 * <p><b>[2026-09-22 정정]</b> 국내 체결 레코드는 원안(2026-06-23 실측, 46필드) 대비 프로덕션 DLQ 원본 페이로드 재대조로 47필드임이 확인됐다
 * — index 0~45는 원안({@code MKSC_SHRN_ISCD}~{@code ABCD_MNPRC})과 완전히 동일하고, KIS가 그 뒤에 미확인 트레일링 필드
 * 1개(index 46, 실측 전부 {@code 2})를 추가로 보낸다. 국내 호가도 동일 사유로 62→63필드로 정정한다(패스스루라 필드 식별 불필요).
 *
...
final class KisFrameFixtures {
    ...
    /**
     * 국내 체결 `H0STCNT0` 레코드 1건 (47필드 — ws-01 실측 원문 46필드 + 2026-09-22 정정 트레일링 필드 1개).
     *
     * <p>{@code [0]}~{@code [45]}는 ws-01 실측 원문과 바이트 동등, {@code [46]}이 새로 추가된 미확인 필드다.
     */
    ...
    /** 국내 체결 레코드의 필드 수 (REQ-032 표, 2026-09-22 정정 — 46→47). */
    static final int DOMESTIC_TRADE_FIELD_COUNT = 47;

    /** 해외 체결 레코드의 필드 수 (REQ-032 표). */
    static final int OVERSEAS_TRADE_FIELD_COUNT = 26;

    /** 국내 호가 레코드의 필드 수 (REQ-032 표, ws-02 실측, 2026-09-22 정정 — 62→63). */
    static final int DOMESTIC_QUOTE_FIELD_COUNT = 63;
